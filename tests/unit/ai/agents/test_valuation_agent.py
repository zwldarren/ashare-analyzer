...
class TestValuationAgent:
    ...
    async def test_analyze_undervalued_returns_buy(self):
        """Test undervalued stock returns BUY signal (heuristic fallback)."""
        with patch(LLM_CLIENT_PATH, return_value=None):
            agent = ValuationAgent()
            ...
            assert "公允价值" in result.reasoning or "fair value" in result.reasoning.lower()

    @pytest.mark.asyncio
    async def test_analyze_overvalued_returns_sell(self):
        """Test overvalued stock returns SELL signal (heuristic fallback)."""
        with patch(LLM_CLIENT_PATH, return_value=None):
            agent = ValuationAgent()
    ...
    async def test_analyze_fairly_valued_returns_hold(self):
        """Test fairly valued stock returns HOLD signal (heuristic fallback)."""
        with patch(LLM_CLIENT_PATH, return_value=None):
            agent = ValuationAgent()
...
class TestValuationAgentLLM:
    ...
    async def test_analyze_with_llm_success(self):
        ...
        mock_llm_client.generate_with_tool = AsyncMock(
            return_value={
                "signal": "buy",
                "confidence": 85,
                "reasoning": "Strong margin of safety at 30% undervaluation",
                "key_metrics": {"margin_of_safety": 30.0},
            }
        )
    ...
    @pytest.mark.asyncio
    async def test_analyze_llm_fallback_to_heuristic(self):
    ...
    @pytest.mark.asyncio
    async def test_analyze_llm_invalid_format_fallback(self):
        """Test fallback when LLM returns invalid format (no 'signal' key)."""
        mock_llm_client = MagicMock()
        mock_llm_client.is_available.return_value = True
        mock_llm_client.generate_with_tool = AsyncMock(
            return_value={"invalid": "format"}  # No 'signal' key
        )
        ...
        with patch(LLM_CLIENT_PATH, return_value=mock_llm_client):
            ...
            # Should fallback to heuristic
            assert result.metadata.get("analysis_method") == "heuristic_fallback"
            assert result.signal in [SignalType.BUY, SignalType.HOLD, SignalType.SELL]
...
class TestValuationAgentMetadata:
    ...
    async def test_metadata_includes_valuations(self):
        """Test metadata includes valuations dict (heuristic path)."""
        with patch(LLM_CLIENT_PATH, return_value=None):
            agent = ValuationAgent()
    ...
    async def test_metadata_includes_method_weights(self):
        """Test metadata includes method_weights dict (heuristic path)."""
        with patch(LLM_CLIENT_PATH, return_value=None):
            agent = ValuationAgent()
    ...
    async def test_metadata_includes_confidence_decay(self):
        """Test metadata includes confidence_decay field (heuristic path)."""
        with patch(LLM_CLIENT_PATH, return_value=None):
            agent = ValuationAgent()
...
class TestValuationAgentHeuristicMarginSignal:
    ...
    def test_margin_to_signal_strong_buy(self):
        ...
        with patch(LLM_CLIENT_PATH, return_value=None):
            ...
            signal, confidence = agent._heuristic_margin_signal(0.20)
    ...
    def test_margin_to_signal_strong_buy_with_excess(self):
        ...
        with patch(LLM_CLIENT_PATH, return_value=None):
            ...
            signal, confidence = agent._heuristic_margin_signal(0.30)
    ...
    def test_margin_to_signal_moderate_buy(self):
        ...
        with patch(LLM_CLIENT_PATH, return_value=None):
            ...
            signal, confidence = agent._heuristic_margin_signal(0.15)
    ...
    def test_margin_to_signal_hold(self):
        ...
        with patch(LLM_CLIENT_PATH, return_value=None):
            ...
            signal, confidence = agent._heuristic_margin_signal(0.0)
    ...
    def test_margin_to_signal_hold_slight_negative(self):
        ...
        with patch(LLM_CLIENT_PATH, return_value=None):
            ...
            signal, confidence = agent._heuristic_margin_signal(-0.03)
